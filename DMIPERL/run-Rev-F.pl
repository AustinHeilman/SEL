...
#
# Austin Heilman, 6/07/07 -REV. E
# Automatically show a model's product code after the MNAME.
# Verifies if the PCODE is already in the MNAME.
# Added functions GetColID and GetColName
#
...
$PERL_VERSION_ID = "0F"; # Hex number. Note: started at 0A
...
{
	# mcodes
	$tmp = &GetStructureField("OEM string 3");
	if ( $tmp =~ /(.{0,4})(.{0,8})(.{0,12}).*/ )
	{
		$dmi{"pmcode"} = $1;
		$dmi{"hwmcode"} = $2;
	...
	# pcode and serial
	$tmp = &GetStructureField("System serial number");
	if ($tmp =~ /(.*)-(.*)/)
	{
		$dmi{"pcode"} = $1;
		$dmi{"serial"} = $2;
	...
	#
	$dmi{"mname"} = &GetStructureField("System product name");
	$dmi{"blid"} = &GetStructureField("OEM string 1");
	$dmi{"uuid"} = &GetStructureField("UUID");
	$dmi{"stag"} = &GetStructureField("System version");
...
sub GetStructureField
{
	...
	{
		local($line) = $_;
		if ( $line =~ m/^$fieldname\.*(.*)/i )
		{
			$line = $1;
			local(@temp) = split(/=\s*/, $line);
			$line = @temp[scalar(@temp)-1];
...
.
	...
	$pcode_id = &GetColID("PCODE");
	$mname_id = &GetColID("MNAME");
	$model_id = &GetColID("MODEL");
	$mcode_id = &GetColID("SWMCODE");
	...
	for ( @modelslines )
	{
		...
		@fields = split(/,/, $_);
		if ( $fields[$pcode_id] eq "?" )
		{
			# CTO model. Avoid the regex nested ? blues.
			$mname = $fields[$model_id]." (CTO)";
		}
		else
		{
			local($num_matches) = &NumColsMatching("MODEL", $fields[$model_id]);
			if ( $num_matches > 1 )
			{
				# Add the product code to the model name.
				$mname = $fields[$model_id]." (".$fields[$pcode_id].")";
			}
			else
			{
				#if ( $fields[$pcode_id] =~ m/$fields[$model_id]/i )
				#{
				#	# Product code is already in the MODEL column (needs to be removed).
				#	$mname = $fields[$model_id];
				#	$mname =~ s/$fields[$pcode_id]//gi;
				#}	
				$mname = $fields[$model_id];
			}
		}
...
# Convert a column position number to a name
sub GetColName
{
	local($col_num) = @_;
	local($first_line) = @modelslines[0];
	local(@fields) = split(/,/, $first_line);
	
	local($tmp) = @fields[$col_num];
	if ( $tmp eq "" )
	{
		$tmp = "ERROR NO COL NAME FOR $col_num";
	}
	return $tmp;
}

# Convert a column name to its position number.
sub GetColID
{
	local($col_name) = @_;
	$col_name =~ tr/a-z/A-Z/;
	...
	for ( local($i)=0; $i<=scalar(@fields); $i++ )
	{
		local($tmp) = @fields[$i];
		$tmp =~ tr/a-z/A-Z/;
		if ( $col_name eq $tmp )
...
# Find columns in the database with duplicate values.
# Useful to check for multiple model name definitions.
sub NumColsMatching
{
	local($col_name, $col_value) = @_;
	local($count) = 0;
	local($col_id) = &GetColID($col_name);
	if ( $col_id < 0 )
	{
		return -1;
	}
	$col_value =~ tr/a-z/A-Z/;
	for ( $i=1; $i<=scalar(@modelslines); $i++ )
	{
		local(@fields) = split(/,/, $modelslines[$i]);
		local($tmp_val) = @fields[$col_id];
		$tmp_val =~ tr/a-z/A-Z/;
		if ( $tmp_val eq $col_value )
		{
			$count++;
		}
	}
