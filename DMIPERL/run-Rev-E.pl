...
$PERL_VERSION_ID = "0E"; # Hex number. Note: started at 0A
...
.
	...
	for ( @modelslines )
	{
		...
		elsif ( $fields[$pcode_id] =~ m/$fields[$model_id]/i )
		{
			# Product code is already in the MODEL column.
			$mname = $fields[$model_id]."'";
		}
		else
		{
			# Add the product code to the mode name.
