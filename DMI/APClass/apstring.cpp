...
/* To-do:
 Implement
apstring::find_first_of
apstring::find_first_not_of
apstring::find_last_of
apstring::find_last_not_of
apstring::rfind
*/
...
void apstring::Pad(char pad, int num)
{
...
//			   otherwise returns npos
{
	...
	{
		if ( strncmp(myCstring + k,str.c_str(),len) == 0 )
			return k;
	}
	return npos;
}
...
int apstring::rfind(const apstring &str) const
{
	int len = str.length();

	for ( int k=length(); k>=str.length(); k-- )
	{
		if ( strncmp(myCstring+k, str.c_str(), len) == 0 )
			return k;
	}
...
int apstring::find_first_of(const apstring& str, int begin)
{
	for ( int i=begin; i<length(); i++ )
	{
		for ( int j=0; j<str.length(); j++ )
		{
			if ( myCstring[i] == str[j] )
				return i;
		}
	}
	return npos;
...
int apstring::find_first_not_of(const apstring& str, int begin)
{
	for ( int i=begin; i<length(); i++ )
	{
		bool match=false;
...
int apstring::find_last_of(const apstring& str)
{
...
int apstring::find_last_not_of(const apstring& str)
{
	for ( int i=(length()-1); i>=0; i-- )
	{
		bool match = false;
...
apstring& apstring::append(const apstring& str)
{
	*this += str;
	return *this;
}

apstring& apstring::append(const apstring& str, int index, int len)
{
	*this += str.substr(index, len);
	return *this;
}

int apstring::compare(const apstring &str)
{
	if ( *this < str )
		return -1;
	else if ( *this > str )
...
bool apstring::empty() const
{
	return (length()<=0);
}

void apstring::swap(apstring &from)
{
	apstring tmp = *this;
	*this = from;
	from = tmp;
